...
const DELTA_ROWS = [
  ...
  "Residential $ Ops Last 7 Days",
  "Commercial $ Ops Last 7 Days"
];
...
const comOpsLast7 = await fetchCommercialOpsLast7Days(dateHeaders);
 ...
 for (const date of dateHeaders) {
  ...
  return {
    ...
    "Residential $ Ops Last 7 Days": resOpsLast7["Residential $ Ops Last 7 Days"],
"Commercial $ Ops Last 7 Days": comOpsLast7["Commercial $ Ops Last 7 Days"],
...
function patchTableWithOverrides(overrides, measurableRows, dateHeaders) {
  ...
  Array.from(table.tBodies[0].rows).forEach(row => {
    ...
    if (!measCell) return;
    const measurable = measCell.textContent.trim();
