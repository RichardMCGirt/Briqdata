...
async function fetchAirtableData9() {
  console.log("📡 Fetching from Airtable with pagination and filter...");
  ...
  // Airtable filterByFormula for "Date Marked Completed" in last 30 days
  const filterFormula = encodeURIComponent(
    "IS_AFTER({Date Marked Completed}, DATEADD(TODAY(), -30, 'days'))"
  );

  do {
    const fetchUrl = `${url}?pageSize=100&filterByFormula=${filterFormula}${offset ? `&offset=${offset}` : ''}`;
...
async function createChart() {
    ...
    const filtered = records.filter(
      rec => rec.fields["Sum of # Elevation and # of Options "] !== undefined && rec.fields["Estimator formula"]
    );
    ...
    for (const rec of filtered) {
      ...
      const value = rec.fields["Sum of # Elevation and # of Options "];
